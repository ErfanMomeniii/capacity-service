...
class TestCapacityAPI:
    ...
    async def test_capacity_endpoint_date_range_validation(self, app_client):
        # date_from > date_to
        response = await app_client.get("/capacity?date_from=2024-03-31&date_to=2024-01-01")
        assert response.status_code == 400

    async def test_capacity_endpoint_future_dates(self, app_client):
        future_date = date.today() + timedelta(days=365)
        future_date_to = future_date + timedelta(days=90)
        response = await app_client.get(
            f"/capacity?date_from={future_date}&date_to={future_date_to}"
        )
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
